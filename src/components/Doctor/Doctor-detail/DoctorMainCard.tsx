...
function DoctorMainCard({ doctor }: DoctorMainCardProps) {
  ...
  const specialty = doctor.specialty || "Chưa cập nhật";
        ...
        <div className="flex-grow">
          <div className="flex justify-between items-start gap-4">
            <div>
              ...
              {doctor.position && <p className="text-slate-500 mt-2">{doctor.position}</p>}
            </div>
          </div>

          <div className="mt-4 flex flex-wrap gap-3">
            <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-md text-xs font-medium">
              {specialty}
            </span>
