  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Danh sách yêu thích</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Quản lý các bác sĩ và cơ sở y tế bạn quan tâm</p>
        </div>
      ...
      <div className="min-h-[400px]">
        {isLoading ? (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-red-100 dark:border-red-900/30 p-12 text-center transition-colors">
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-2 mb-6">Không thể tải danh sách yêu thích của bạn.</p>
            <button
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-20 text-center transition-colors">
            <div className="w-20 h-20 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-6">
              <span className="material-symbols-outlined text-4xl text-slate-300 dark:text-slate-600">
                {activeTab === 'doctors' ? 'person_off' : 'domain_disabled'}
              </span>
            </div>
            <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100">Danh sách trống</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredFavorites.map((item) => {
              if (activeTab === 'doctors' && item.doctor) {
                ...
                return (
                  <div key={item.id} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 hover:shadow-lg transition-all group relative">
                    <div className="flex gap-4">
                      <div className="relative">
                        <img
                          src={doctor.avatarUrl || 'https://via.placeholder.com/150'}
                          alt={doctor.fullName}
                          className="w-20 h-20 rounded-2xl object-cover bg-slate-50 dark:bg-slate-800"
                        />
                      </div>
              ...
              if (activeTab === 'hospitals' && item.hospital) {
                ...
                return (
                  <div key={item.id} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 hover:shadow-lg transition-all group relative">
                    <div className="flex gap-4">
                      <div className="w-20 h-20 rounded-2xl bg-slate-50 dark:bg-slate-800 overflow-hidden flex items-center justify-center flex-shrink-0">
                        {hospital.logoUrl ? (
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                              isClinic ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' : 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'
                            }`}>
                              {isClinic ? 'Phòng khám' : 'Bệnh viện'}
                            </span>
                            <h3 className="font-bold text-slate-900 dark:text-slate-100 line-clamp-2 mt-2 leading-snug">
                              {hospital.name}
                            </h3>
                          </div>
